...
function doStuff(report, reportId) {
  ...
  //What was the NK?
  //When did the NK Die?
  //How many Town Were alive when NK died?
  //How many Maf Were alive when NK died?
  //Was NE alive when NK died?
  //If yes, what was it?
  //Which Factions Won?
  let NK = null 
  let NE = null
  let When_did_the_NK_die = null
  let Town_that_were_alive = null
  let Mafia_that_were_alive = null
  let Was_NE_alive_when_NK_died = null
  let Which_faction_won = null
  let Did_NE_survive_to_end = null
  let Did_NE_win = null
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    if (entry.type == 'TRIAL OUTCOME') {
      let trial = entry
      if (players[entry.accused].role == 'Jester' && trial.outcome == 'lynched') {
        Did_NE_win = true
      }
      if (players[entry.accused].role == NK && trial.outcome == 'lynched') {
        When_did_the_NK_die = trial.time.split('.')[0]
        for (let j = 0; j < trial.votes.length; j++) {
          let vote = trial.votes[j]
          Mafia_that_were_alive += (players[vote.author].faction == 'Mafia')
          Town_that_were_alive += (players[vote.author].faction == 'Town' && !vote.mayor)
        }
      }
      for (let name in players) {
        if (players[name].role == NE) {
          if (players[name].killed.length) {
            Was_NE_alive_when_NK_died = (Number(players[name].killed[0].split('.')[1]) > Number(trial.time.split('.')[1]))
            Did_NE_survive_to_end = false
          } else {
            Was_NE_alive_when_NK_died = true
            Did_NE_survive_to_end = true
          }
        }
      }
    }
    if (entry.type == 'HAS BEEN KILLED') {
      if (players[entry.player].role == 'Jester') {
        if (players[entry.player].killed.length) {
          if (players[entry.player].killed[0].split('.')[0][0] == 'N') {
            Did_NE_win = false
          }
        }
      }
      if (players[entry.player].role == NK) {
        if (When_did_the_NK_die == null) {
          for (let name in players) {
            if (players[name].role == NK) {
              When_did_the_NK_die = players[name].killed[0].split('.')[0]
            }
            if (players[name].faction == 'Mafia') {
              if (players[name].killed.length) {
                let now = Number(entry.time.split('.')[1])
                let killed = Number(players[name].killed[0].split('.')[1])
                Mafia_that_were_alive += (now < killed)
              } else {
                Mafia_that_were_alive++
              }
            }
            if (players[name].faction == 'Town') {
              let player = players[name]
              let now = Number(entry.time.split('.')[1])
              let firstDeath, secondDeath, resurrected
              if (!player.killed.length) {
                Town_that_were_alive++
              }
              if (player.killed.length) {
                ...
                if (player.resurrected) {
                  resurrected = player.resurrected.split('.')[1]
                }
                ...
                if (player.killed.length == 2) {
                  if (resurrected < now && secondDeath > now) {
                    Town_that_were_alive++
                  }
                }
              }
    ...
    if (entry.type == 'SYSTEM' && entry.text.endsWith(' has won.')) {
      Which_faction_won = entry.text.split(' ')[0]
      if (entry.text.split(' ')[0] != 'Town' && entry.text.split(' ')[0] != 'Mafia' && entry.text.split(' ')[0] != 'Arsonist' && entry.text.split(' ')[0] != 'Werewolf' && entry.text.split(' ')[0] != 'SerialKiller')
      console.log(`${JSON.stringify(entry)}`.red)
    }
    if (entry.type == 'SYSTEM') {
      switch (entry.text) {
        case 'Draw.':
          Which_faction_won = 'Draw'
          break
        case 'Stalemate.':
          Which_faction_won = 'Stalemate'
          break
      }
    }
  }

  if (Which_faction_won === null) {
    console.log(`GAME OUTCOME UNDEFINED`.red)
  }

  if (Did_NE_survive_to_end && NE == 'Witch' && Which_faction_won != 'Town') {
    Did_NE_win = true
  }
  ...
  if (!Did_NE_survive_to_end && NE == 'Witch') {
    Did_NE_win = false
  }

  if (Did_NE_survive_to_end && NE == 'Jester') {
    Did_NE_win = false
  }

  NE = NE.replace('Jester', 'Jester (Possibly started as Executioner)')

  OUTPUT.reportId = reportId
  OUTPUT.NK = NK
  OUTPUT.NE = NE
  OUTPUT.When_did_the_NK_die = When_did_the_NK_die
  OUTPUT.Town_that_were_alive = Town_that_were_alive
  OUTPUT.Mafia_that_were_alive = Mafia_that_were_alive
  OUTPUT.Was_NE_alive_when_NK_died = Was_NE_alive_when_NK_died
  OUTPUT.Did_NE_survive_to_end = Did_NE_survive_to_end
  OUTPUT.Did_NE_win = Did_NE_win
  OUTPUT.Which_faction_won = Which_faction_won

  stream2.write(JSON.stringify(OUTPUT, null, 0) + ',\n', (() => { }))

  matchesProcessed++
  console.log(reportId + ' ' + matchesProcessed + `${JSON.stringify(OUTPUT, null, 2)}`.green)
}
